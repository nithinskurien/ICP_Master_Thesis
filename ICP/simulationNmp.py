...
if __name__ == '__main__':
    ...
    var_init = [q, q]
    runs = 1
    collectMSE = [[None for j in range(1, rangeNmp)] for i in range(n_v)]
    ...
    for n_mp in range(1, rangeNmp):
        ...
        for run in range(runs):
            ...
            init_feat_distr = []
            for f in range(n_f):
                init_feat_distr_mean = np.reshape(np.random.multivariate_normal(np.array(p3).flatten(), cov3, 1),
                                                  [2, 1])
    ...
    pos_est0 = data[0].get_veh_belief(0)
    pos_updt0 = data[0].get_updt_veh_belief(0)
    pos_pred0 = data[0].get_pred_veh_belief(0)
    pos_est1 = data[0].get_veh_belief(1)
    pos_updt1 = data[0].get_updt_veh_belief(1)
    pos_pred1 = data[0].get_pred_veh_belief(1)
