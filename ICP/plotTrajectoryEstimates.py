...
#read the pickle
name = '2_1_0.01_0.0001_0.01'
nameM = '2_3_0.01_0.0001_0.01'
N_mp = 2
...
with open('/home/tomek/TheTitans/ICP/Pickles/dataKalman' + nameM + '.pickle', 'rb') as input:
...
plt.plot(np.array(meas.mean_veh[0][0]).flatten(),
         np.array(meas.mean_veh[0][1]).flatten(), '--g',
         label='Actual trajectory', linewidth=3, alpha=0.9)
plt.plot(np.array(meas.meas_veh[0][0]).flatten()[1:],
         np.array(meas.meas_veh[0][1]).flatten()[1:], 'ok',
...
plt.plot(pos_est0Kalman[0][0], pos_est0Kalman[1][0], 'xr', label='Initial estimate', markersize=10,  mew=4, linewidth=2.5, alpha=0.9)
plt.plot(pos_est0ICP[0], pos_est0ICP[1], 'b', label='Position estimate ICP', linewidth=2.5, alpha=0.9)
...
plt.plot(np.array(meas.mean_veh[1][0]).flatten(),
         np.array(meas.mean_veh[1][1]).flatten(), '--g',
         label='Actual trajectory', linewidth=3, alpha=0.9)
plt.plot(np.array(meas.meas_veh[1][0]).flatten()[1:],
         np.array(meas.meas_veh[1][1]).flatten()[1:], 'ok',
         label='GNSS measurement', linewidth=1.5, alpha=0.8, fillstyle=None)
plt.plot(pos_est1Kalman[0][0], pos_est1Kalman[1][0], 'xr', label='Initial estimate', markersize=10,  mew=4, linewidth=2.5, alpha=0.9)
plt.plot(pos_est1Kalman[0], pos_est1Kalman[1], 'r', label='Position estimate Kalman', linewidth=2.5, alpha=0.9)
plt.plot(pos_est1ICP[0], pos_est1ICP[1], 'b', label='Position estimate ICP', linewidth=2.5, alpha=0.9)
...
veh1fig.savefig('/home/tomek/TheTitans/ICP/Figures/trajectory2_veh1_' +name+ '_' + str(N_mp) + '.eps')
veh2fig.savefig('/home/tomek/TheTitans/ICP/Figures/trajectory2_veh2_' +name+ '_' + str(N_mp) + '.eps')
