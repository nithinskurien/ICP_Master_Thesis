...
if __name__ == '__main__':
    ...
    cov3 = np.matrix('25 0; 0 25')

    p4 = np.matrix('0.5; 0.5')
    cov4 = np.matrix('25 0; 0 25')

    p5 = np.matrix('0.5; 0.5')
    cov5 = np.matrix('25 0; 0 25')

    p6 = np.matrix('0.5; 0.5')
    cov6 = np.matrix('25 0; 0 25')

    p7 = np.matrix('0.5; 0.5')
    cov7 = np.matrix('25 0; 0 25')
    ...
    var_init = [q, q]
    runs = 100
    dataKalman = [None for i in range(runs)]
    ...
    for run in range(runs):
        ...
        dataKalman[run] = DataCollect.DataCollect(n_v, n_f)
        dataKalman[run].init(n_v, n_f, init_veh_distr, init_feat_distr[:n_f])
        v = algorithmKalman.init_veh(n_v, n_f, init_veh_distr, init_feat_distr[:n_f], var_init)
        algorithmKalman.main_loop(v, meas[run], 1, dataKalman[run])
        for k in range(n_v):
            error_square[k][run] = np.power(dataKalman[run].get_veh_belief(k)[:2, :] - meas[run].mean_veh[k], 2)
    ...
    with open('/home/tomek/TheTitans/ICP/Pickles/errorKalman' +
              ...
              '_' + str(runs) + '.pickle', 'wb') as output:
    ...
    pos_est0 = dataKalman[0].get_veh_belief(0)
    pos_updt0 = dataKalman[0].get_updt_veh_belief(0)
    pos_pred0 = dataKalman[0].get_pred_veh_belief(0)
    pos_est1 = dataKalman[0].get_veh_belief(1)
    pos_updt1 = dataKalman[0].get_updt_veh_belief(1)
    pos_pred1 = dataKalman[0].get_pred_veh_belief(1)
    ...
    plt.plot(np.concatenate(([None], np.array(meas[0].meas_veh[0][0]).flatten()), axis = 0), 'ok',
    ...
    plt.plot(np.concatenate(([None], np.array(meas[0].meas_veh[0][1]).flatten()), axis=0), 'ok',
             label='GNSS measurement', fillstyle='full')
    ...
    plt.plot(np.concatenate(([None], np.array(meas[0].meas_veh[1][0]).flatten()), axis = 0), 'ok',
    ...
    plt.plot(np.concatenate(([None], np.array(meas[0].meas_veh[1][1]).flatten()), axis = 0), 'ok',
