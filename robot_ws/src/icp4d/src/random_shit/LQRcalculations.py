...
f1 = x + sp.cos(theta)*(v/phi)*sp.sin(phi*ts) - sp.sin(theta)*(v/phi)*(1-sp.cos(phi*ts))
f2 = v*sp.cos(theta)
f3 = y + sp.sin(theta)*(v/phi)*sp.sin(phi*ts) + sp.cos(theta)*(v/phi)*(1-sp.cos(phi*ts))
f4 = v*sp.sin(theta)
f5 = theta + phi*ts
f6 = phi
A = [[None for i in range(3)] for j in range(3)]
B = [[None for i in range(2)] for j in range(3)]
# f = [f1, f2, f3, f4, f5, f6]
...
for i in range(len(f)):
    for j in range(len(xvec)):
        A[i][j] = sp.diff(f[i], xvec[j])
print A

for i in range(len(f)):
    for j in range(len(uvec)):
        B[i][j] = sp.diff(f[i], uvec[j])
...
# (ts*v*np.sin(phi*ts + theta) + y)/phi - (phi*y + v*np.cos(theta) - v*np.cos(phi*ts + theta))/phi**2],
#      [0, ts]])
#
# Q = np.matrix([[1, 0, 0],
#                [0, 1, 0],
