...
def consensus_fnc(veh):
    ...
    for v in veh:
        for f_id in v.visible_feat:
            consensus_cov = np.linalg.inv(temp_covinv[f_id])
            consensus_mu = np.dot(consensus_cov, temp_covinvmu[f_id])
            # print consensus_cov
            # print consensus_mu
            v.feat[f_id].consensus = Distribution.Distribution(consensus_mu, consensus_cov)
