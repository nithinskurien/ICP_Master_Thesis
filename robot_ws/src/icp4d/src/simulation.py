...
if __name__ == '__main__':
    ...
    init_feat_distr = [Distribution.Distribution(p3, cov3) for i in range(n_f)]
    ...
    meas = me.Measurement(n_v, n_f, t_s)
    ...
    #     meas[run] = me.Measurement(n_v, n_f)
    #     data[run] = DataCollect.DataCollect(n_v, n_f)
    #     data[run].init(n_v, n_f, init_veh_distr, init_feat_distr[:n_f])
    #     v = algorithm.init_veh(n_v, n_f, init_veh_distr, var_init)
    #     algorithm.main_loop(v, meas[run], 2, 1, data[run])
    #     for k in range(n_v):
    #         error_square[k][run] = np.power(data[run].get_veh_belief(k)[:2, 1:] - meas[run].mean_veh[k], 2)
