...
def callback(rob0, rob1):
    ...
    if count==0:  # re-route function activated if no readings received in 2 secs
        ...
        tra_mod, tra_samp = traj.Trajectory(1.7, 1.1, 1.0).re_route(ctrl_x, ctrl_y, euler_cor, loop)
        tra_start = t_present
        prev_input = np.matrix([[0.1], [0.1]])
...
def print_plots():
    ...
    plt.grid(True)
    plt.title('Trajectory tracking Robot0')
    plt.xlabel('X (m)')
    plt.ylabel('Y (m)')
    plt.xlim((0.5, 3.0))
    plt.ylim((-0.5, 3.0))
    ...
    plt.savefig('Robot_0_Trajectory.eps')
    fig2 = plt.figure()
    ...
    plt.grid(True)
    plt.title('$\Theta$ tracking Robot0')
    ...
    plt.grid(True)
    plt.legend()
    plt.title('Distance error Robot0')
    plt.savefig('Robot_0_Error.eps')
    plt.show()


def listener():
    rospy.init_node('Robot_Controller0', anonymous=True)
    global loop
    true_input = False
