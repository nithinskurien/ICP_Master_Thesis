...
int main(int argc, char** argv) {
   ...
   while (1) {
      ...
      if (detections.empty()) {
         ...
         double camera_Positionx,camera_Positiony,direction;
	...
	//Camera Matrix
         ...
         if(detections.size()>3)
        {
	     ...
	     direction = euler[2]*1000000;
         std::cout << "Heading" <<" " << (int)direction << std::endl;
         std::cout << "Camera Position x" <<" " << camera_Positionx << std::endl;
         std::cout << "Camera Position y" <<" " << camera_Positiony << std::endl;

        }
         int count = 0;
         for (size_t i=0; i<detections.size(); ++i) {
                     TagDetection &dd = detections[i];
                     if (dd.id == 0 or dd.id == 1 or dd.id == 2 or dd.id == 3) {
                    	 count++;
                     }
         }
         for (size_t i=0; i<detections.size(); ++i) {
	       ...
	       //Finding relative position of robot with respect to the Tags
               if(count > 3 and dd.id == 4)
               {
            	   x_pos[0] = (x_new-camera_Positionx) * 1000.0;
            	   y_pos[0] = (y_new-camera_Positiony) * 1000.0;
            	   angle[0] = direction;
            	   std::cout << "inside 4"  << std::endl;
               }
               if(count > 3 and dd.id == 5)
               {
            	   x_pos[1] = (x_new-camera_Positionx) * 1000.0;
            	   y_pos[1] = (y_new-camera_Positiony) * 1000.0;
            	   angle[1] = direction;
